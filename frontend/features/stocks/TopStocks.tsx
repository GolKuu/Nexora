...
export function TopStocks({ limit = 12 }: { limit?: number }) {
  ...
  const subtitle = data?.latest_market_timestamp
    ? `Реальные данные KASE на ${formatDate(data.latest_market_timestamp)} · рейтинг обновляется автоматически`
    : "Реальные акции KASE: качество, оценка, дивиденды, ликвидность и риск.";
  return <Card><CardHeader title="Акции KASE" subtitle={subtitle} />
