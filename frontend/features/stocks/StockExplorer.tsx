...
export function StockExplorer() {
  ...
  const catalogStatus = !data && isLoading
    ? "Загрузка каталога…"
    : error
      ? "Не удалось загрузить список акций"
      : `${items.length} из ${data?.total ?? 0} · цена, изменение, источник и свежесть данных`;
