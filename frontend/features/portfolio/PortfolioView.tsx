  ...
  return (
    <div className="grid gap-3 sm:grid-cols-4">
      <Field label="Тикер или ISIN">
        <Input value={bond} onChange={(e) => setBond(e.target.value)} placeholder="DBNKb1" />
      </Field>
      ...
      <Field label="Цена покупки, % номинала" hint="можно не указывать">
        <Input
          type="number"
          step="0.01"
          value={price}
      ...
      <div className="flex items-end">
        <Button onClick={() => void submit()} disabled={busy || !bond.trim()} className="w-full">
          Добавить
        </Button>
      </div>
            ...
            <table className="w-full min-w-[640px] text-sm">
              ...
              <tbody>
                {data.positions.map((position) => (
                  ...
                  >
                    <td className="py-2">
                      <Link href={`/bond/${position.ticker}`} className="font-medium hover:underline">
                        {position.ticker}
                      </Link>
                    ...
                    <td className="tabular py-2 text-right">
                      {formatNumber(position.clean_price, 2)}
                    </td>
                    <td className="tabular py-2 text-right">
                    ...
                    <td className="tabular py-2 text-right">{formatRate(position.ytm)}</td>
