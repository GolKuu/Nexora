...
export function BondExplorer() {
  const { data, isLoading, error } = useSWR("bonds-full-catalog", () => bondsService.list({ limit: 200 }), { refreshInterval: 60_000 });
  ...
  return <Card><CardHeader title="Все доступные облигации" subtitle={`${items.length} из ${data?.total ?? 0} · YTM, купон, погашение, кредит и ликвидность`}/>
