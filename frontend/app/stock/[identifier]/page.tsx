...
const TechnicalAnalysisPanel = dynamic(
  () => import("@/features/stocks/TechnicalAnalysisPanel").then(module => module.TechnicalAnalysisPanel),
  { ssr: false, loading: deferredPanel },
);
...
export default function StockPage({params}:{params:Promise<{identifier:string}>}) {
  ...
  return <div className="space-y-4">
    ...
    <TechnicalAnalysisPanel ticker={data.ticker} currency={data.currency}/>
    <div className="grid gap-4 lg:grid-cols-3"><div className="space-y-4 lg:col-span-2">
      ...
      <ScoreHistoryPanel identifier={data.ticker}/>
      ...
      {data.corporate_actions.length?<Card><CardHeader title="Корпоративные события" subtitle="Дивиденды, buyback, выпуск акций, split, M&A, листинг и рейтинговые события."/><CardBody className="space-y-3">{data.corporate_actions.slice(0,10).map((action,index)=><div key={`${action.action_type}-${action.event_date}-${index}`} className="flex items-start justify-between gap-3"><span><span className="block text-sm font-medium">{action.title}</span><span className="block text-xs text-slate-500">{action.action_type} · {action.status} · {formatDate(action.event_date)}</span></span>{action.source_url?<a href={action.source_url} target="_blank" rel="noreferrer" className="shrink-0 text-xs underline">источник ↗</a>:null}</div>)}</CardBody></Card>:null}
