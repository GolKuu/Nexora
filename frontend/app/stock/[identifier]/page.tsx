...
export default function StockPage({params}: {params: Promise<{identifier: string}>}) {
  ...
  return <div className="space-y-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><h1 className="text-3xl font-semibold">{data.ticker}</h1><Badge tone="success">{data.type_label}</Badge></div><p className="mt-1 text-sm text-slate-500">{data.company_name} · данные {formatDate(data.data_timestamp)}</p></div><a href={data.kase_url ?? "#"} target="_blank" rel="noreferrer" className="rounded-xl border border-slate-200 px-3 py-2 text-sm dark:border-slate-700">Открыть на KASE ↗</a></div>
    <div className="grid gap-4 lg:grid-cols-3"><div className="space-y-4 lg:col-span-2"><Card><CardBody className="grid gap-5 sm:grid-cols-[auto_1fr]"><ScoreDial value={data.scores.investment?.value} label="из 100" caption="Общая оценка" /><div><p className="text-3xl font-semibold tabular">{formatMoney(data.price, data.currency, 2)}</p><p className="mt-1 text-sm text-slate-500">{data.simple.valuation}. {data.simple.important}</p><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">{Object.entries(SCORE_LABELS).map(([kind, label]) => <ScoreBar key={kind} label={label} value={data.scores[kind]?.value} />)}</div></div></CardBody></Card>
    ...
    </div><div className="space-y-4"><StockCalculator ticker={data.ticker} currency={data.currency} /><Card><CardHeader title="Источник и свежесть" /><CardBody className="space-y-2 text-sm"><p><span className="text-slate-500">Источник:</span> {data.source ?? "—"}</p><p><span className="text-slate-500">На дату:</span> {formatDate(data.data_timestamp)}</p><p className="text-xs text-slate-500">Last не гарантирует цену покупки. Калькулятор предпочитает актуальный ask и предупреждает при fallback.</p></CardBody></Card></div></div></div>;
}
