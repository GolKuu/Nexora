...
| `data_age_seconds` | Возраст **рыночных** данных (не момента загрузки). Есть в ответе калькулятора и в `/health/kase` |
| `is_stale` | `true`, если данные старше порога (3 суток). Тогда `data_mode` автоматически становится `cached` |
...
**Работа без KASE.** API читает из базы, а не с биржи, поэтому все эндпоинты
продолжают отвечать при недоступности KASE. В этом случае:
`/health/kase` → `connected: false`, `mode: "cache"`; данные помечаются
`data_mode: "cached"` с реальным `data_age_seconds`. Показывайте возраст —
цифры остаются настоящими, но перестают быть свежими.
...
**6. `is_estimated`** в потоке означает, что ставка периода еще не
зафиксирована. KASE публикует официальный график купонов
(`/api/instruments/coupon-payments/{TICKER}/`), поэтому у выпусков с
синхронизированным графиком даты и суммы — **факт**, и `is_estimated=false`
даже для плавающего купона: берется ставка каждого периода, а не сегодняшняя.
`true` остается только там, где будущая ставка объективно неизвестна.
...
5. **График купонов официальный**, а не расчетный — см. `is_estimated`.
6. **`credit_spread` может быть `null` у валютных выпусков**: кривая ГЦБ
   строится в тенге, и сравнивать с ней USD-бумагу некорректно.
